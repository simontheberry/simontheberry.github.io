...
export function BusinessStep({ form, updateForm }: Props) {
  const [searchQuery, setSearchQuery] = useState(form.business.name);
  ...
  return (
    <div>
      ...
      {/* Search */}
      <div>
        <label htmlFor="business-search" className="block text-sm font-medium text-gov-grey-700 mb-1">
          Business Name
        </label>
        <div className="flex gap-2">
          ...
          <button
            ...
            aria-label="Search Australian Business Register"
          >
            {isSearching ? (
      ...
      {searchResults.length > 0 && (
        <div className="mt-4 space-y-2" role="listbox" aria-label="Business search results">
          ...
          {searchResults.map((result) => (
            <button
              ...
              role="option"
              aria-selected={form.business.abn === result.abn}
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <Building2 className="h-5 w-5 text-gov-grey-300" aria-hidden="true" />
              </div>
            </button>
          ))}
        </div>
      ...
      {form.business.isVerified && (
        <div className="mt-6 rounded-lg border border-green-200 bg-green-50 p-4">
          <div className="flex items-center gap-2 mb-2">
            <CheckCircle2 className="h-5 w-5 text-gov-green" aria-hidden="true" />
          ...
          <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
            <div>
              ...
              <dd className="font-medium">{form.business.entityName}</dd>
            </div>
            <div>
            ...
            <div>
              ...
              <dd>{form.business.entityType}</dd>
            </div>
            <div>
              <dt className="text-gov-grey-500">Status</dt>
              <dd>{form.business.entityStatus}</dd>
            </div>
          </dl>
          <button
            onClick={() =>
              updateForm((prev) => ({
                ...prev,
                business: { ...prev.business, isVerified: false, abn: '', entityName: '', entityType: '', entityStatus: '' },
              }))
            }
            className="mt-3 text-xs text-gov-grey-500 underline hover:text-gov-grey-700"
          >
            Search again
          </button>
        </div>
      ...
      {!form.business.isVerified && (
        <div className="mt-6 space-y-4">
          <p className="text-xs text-gov-grey-500">Or enter details manually:</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label htmlFor="business-abn" className="block text-sm font-medium text-gov-grey-700 mb-1">
                ABN
              </label>
              <input
                id="business-abn"
                ...
                onChange={(e) =>
                  updateForm((prev) => ({
                    ...prev,
                    business: { ...prev.business, abn: e.target.value },
                  }))
                }
            ...
            <div>
              <label htmlFor="business-website" className="block text-sm font-medium text-gov-grey-700 mb-1">
                Website
              </label>
              <input
                id="business-website"
